...
@Tag(name = "Paquetes Turísticos", 
     description = "Gestión completa de paquetes turísticos y su relación con sitios de interés")
public class PaqueteController {
    ...
    @Operation(summary = "Obtener todos los paquetes",
               description = "Retorna una lista de todos los paquetes turísticos disponibles")
    @ApiResponse(responseCode = "200", 
                 description = "Lista de paquetes encontrada",
                 content = @Content(schema = @Schema(implementation = PaqueteDTO.class)))
    public ResponseEntity<List<PaqueteDTO>> getAll() {
        return ResponseEntity.ok(service.getAll());
    }

    @GetMapping("/con-sitios")
    @Operation(summary = "Obtener paquetes con sitios incluidos",
               description = "Retorna paquetes con la información completa de los sitios turísticos que incluyen")
    @ApiResponse(responseCode = "200", 
                 description = "Lista de paquetes con sitios encontrada",
                 content = @Content(schema = @Schema(implementation = PaqueteConSitiosDTO.class)))
    ...
    @Operation(summary = "Buscar paquetes por sitio",
               description = "Retorna los paquetes que incluyen un sitio turístico específico")
    @ApiResponses({
    ...
    public ResponseEntity<List<PaqueteConSitiosDTO>> getBySitio(
            @Parameter(description = "ID del sitio turístico", example = "1", required = true)
            @PathVariable Integer idSitio) {
        return ResponseEntity.ok(service.findBySitio(idSitio));
    }

    @GetMapping("/{id}")
    @Operation(summary = "Obtener paquete por ID",
               description = "Retorna los detalles básicos de un paquete específico")
    @ApiResponses({
    ...
    public ResponseEntity<PaqueteDTO> getById(
            @Parameter(description = "ID del paquete", example = "1", required = true)
            @PathVariable Integer id) {
        return ResponseEntity.ok(service.getById(id));
    }

    @GetMapping("/{id}/con-sitios")
    @Operation(summary = "Obtener paquete con sitios por ID",
               description = "Retorna un paquete específico con la lista completa de sitios incluidos")
    @ApiResponse(responseCode = "200", 
                 description = "Paquete con sitios encontrado",
                 content = @Content(schema = @Schema(implementation = PaqueteConSitiosDTO.class)))
    ...
    @Operation(summary = "Crear nuevo paquete",
               description = "Registra un nuevo paquete turístico en el sistema")
    @ApiResponses({
        @ApiResponse(responseCode = "201", description = "Paquete creado exitosamente"),
        @ApiResponse(responseCode = "400", description = "Datos de entrada inválidos")
    })
    public ResponseEntity<PaqueteDTO> create(
            @Parameter(description = "Datos del paquete a crear", required = true)
            @RequestBody @Valid PaqueteDTO dto) {
    ...
    public ResponseEntity<PaqueteDTO> update(
            @Parameter(description = "ID del paquete a actualizar", example = "1", required = true)
            @PathVariable Integer id,
            @Parameter(description = "Datos actualizados del paquete", required = true)
            @RequestBody @Valid PaqueteDTO dto) {
    ...
    public ResponseEntity<Void> delete(
            @Parameter(description = "ID del paquete a eliminar", example = "1", required = true)
            @PathVariable Integer id) {
        service.delete(id);
    ...
    @Operation(summary = "Agregar sitio a paquete",
               description = "Añade un sitio turístico a un paquete existente")
    @ApiResponses({
    ...
    public ResponseEntity<Void> addSitio(
            @Parameter(description = "Relación paquete-sitio a crear", required = true)
            @RequestBody @Valid PaqueteSitioDTO dto) {
        service.addSitioToPaquete(dto);
        return ResponseEntity.ok().build();
    }

    @PostMapping("/remover-sitio")
    @Operation(summary = "Remover sitio de paquete",
               description = "Elimina un sitio turístico de un paquete existente")
    @ApiResponses({
    ...
    public ResponseEntity<Void> removeSitio(
            @Parameter(description = "Relación paquete-sitio a eliminar", required = true)
            @RequestBody @Valid PaqueteSitioDTO dto) {
